...
colnames(basis) = str_c("GEP_", c(1:ncol(basis)))

basis = order_factorized_matrix(basis)

Basis_subset = basis[, 23, drop = FALSE]
...
GEP23_CH = read.delim("GEP_23_genes.txt", header = FALSE, col.names = "gene_ID")

Basis_label = Basis_label[GEP23_CH$gene_ID, ]
...
GEP_23_DES = read.csv("Create_gene_description_file/GEP23_genes_description.csv", row.names = 1)
...
Basis_label = merge(Basis_label, GEP_23_DES, by.x = "gene_ID", by.y = "CH_ID")
Basis_label = Basis_label[order(Basis_label$GEP_23, decreasing = FALSE), ]
...
p <- ggplot(Basis_subset, aes(x = index, y = GEP_23)) + geom_point(color = grp_col[3], size = 4) +
  ggrepel::geom_text_repel(data = Basis_label, aes(label = Gene_code_name), nudge_x = -600, size = 16, force = 100) +
...
ggsave(filename = "Figure_1G_1.png", plot = p, width = 10, height = 16, dpi = 300)
