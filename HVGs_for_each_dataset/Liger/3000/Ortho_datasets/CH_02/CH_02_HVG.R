...
OX_2E <- NormalizeData(OX_2E, normalization.method = "RC", scale.factor = 1)


# Selection of Highly Variable Genes (Liger)
OX_HVGs = variableGenes_liger(dataObject = OX_2E, var.thresh = 0.1, num.genes = 3000, alpha.thresh = 0.99, tol = 0.0001)
fileGenerator(OX_HVGs, fileName = "CH_2E_3000_HVG.txt")

mean_var_OX_2E = generate_mean_var_liger(OX_2E)

save(mean_var_OX_2E, file = "Mean_Variance_CH_02.RData")
...
p <- ggplot(data = mean_var_OX_2E, aes(x = avgExpression, y = geneVariance)) + 
  ...
  geom_abline(intercept = log10(mean((1 / OX_2E@meta.data[["nCount_RNA"]]))), slope = 1, color = "red") + 
...
ggsave(plot = p, filename = "OX_2E_Liger_HVG_ab.png", width = 12, height = 12, dpi = 300)
...
ggsave(plot = p, filename = "OX_2E_Liger_HVG.png", width = 12, height = 12, dpi = 300)
