...
GEP_23_DES = read.csv("Cluster_5_GEP_32_markers.csv", row.names = 1)
rownames(GEP_23_DES) <- GEP_23_DES$CH_ID
...
Basis_subset = Basis_subset[order(Basis_subset$GEP_32, decreasing = FALSE), , drop = FALSE]
...
Basis_label = Basis_label[order(Basis_label$GEP_32, decreasing = FALSE), ]
Basis_label$gene_ID = rownames(Basis_label)

# Let's load the description file of the GEP genes
gep_des = read.csv("Cluster_5_GEP_32_markers.csv", row.names = 1)
...
# With the interested one
Basis_label = Basis_label[Basis_label$index %in% c(1965, 1970, 1974, 1975, 1977, 1979, 1982, 1983, 1986, 1987, 1991, 1992, 1993, 1998, 1999, 2000), ]

# Let's take a subset to fit in the figure
Basis_label = Basis_label[Basis_label$index %in% c(1979, 1982, 1987, 1991, 1992, 1993, 1998, 1999, 2000), ]

p <- ggplot(Basis_subset, aes(x = index, y = GEP_32)) + geom_point(color = grp_col[3], size = 4) +
  ggrepel::geom_text_repel(data = Basis_label, aes(label = Gene_code_name), nudge_x = -500, size = 16, force = 10) +
  xlab("GEP 32 genes") + ylab("Coefficient") +
  theme(
...
ggsave(filename = "Figure_3G_1.png", plot = p, width = 10, height = 16, dpi = 300)
