...
length(intersect(geneID_chipseq, GEP40_genes))  # 40 genes
...
write.csv(GEP40_chip_subset, file = "Genes_peak/Chip_subset_GEP40.csv", row.names = FALSE)

# Get the description
description_GEP40_and_Chip = ATH[rownames(ATH) %in% intersect(geneID_chipseq, GEP40_genes), ]

temp_mat = matrix("NA", ncol = ncol(description_GEP40_and_Chip), nrow = length(setdiff(intersect(geneID_chipseq, GEP40_genes), rownames(description_GEP40_and_Chip))))
temp_mat = data.frame(temp_mat)
colnames(temp_mat) = colnames(description_GEP40_and_Chip)

for (i in c(1: length(setdiff(intersect(geneID_chipseq, GEP40_genes), rownames(description_GEP40_and_Chip))))){
  temp_mat$CH_ID[i] = setdiff(intersect(geneID_chipseq, GEP40_genes), rownames(description_GEP40_and_Chip))[i]
}

temp = rbind.data.frame(description_GEP40_and_Chip, temp_mat)
rownames(temp) = temp$CH_ID

# Write a csv file with Chipseq
write.csv(temp, file = "Genes_peak/GEP40_Chip_genes_description.csv", row.names = FALSE)
