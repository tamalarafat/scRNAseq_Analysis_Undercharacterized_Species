...
COL_3E <- NormalizeData(COL_3E, normalization.method = "RC", scale.factor = 1)


# Selection of Highly Variable Genes (Liger)
COL_HVGs = variableGenes_liger(dataObject = COL_3E, var.thresh = 0.1, num.genes = 3000, alpha.thresh = 0.99, tol = 0.0001)
fileGenerator(COL_HVGs, fileName = "AT_3E_3000_HVG.txt")

mean_var_COL_3E = generate_mean_var_liger(COL_3E)

save(mean_var_COL_3E, file = "Mean_Variance_AT_03.RData")
...
p <- ggplot(data = mean_var_COL_3E, aes(x = avgExpression, y = geneVariance)) + 
  ...
  geom_abline(intercept = log10(mean((1 / COL_3E@meta.data[["nCount_RNA"]]))), slope = 1, color = "red") + 
...
ggsave(plot = p, filename = "COL_3E_Liger_HVG_ab.png", width = 12, height = 12, dpi = 300)
...
ggsave(plot = p, filename = "COL_3E_Liger_HVG.png", width = 12, height = 12, dpi = 300)
