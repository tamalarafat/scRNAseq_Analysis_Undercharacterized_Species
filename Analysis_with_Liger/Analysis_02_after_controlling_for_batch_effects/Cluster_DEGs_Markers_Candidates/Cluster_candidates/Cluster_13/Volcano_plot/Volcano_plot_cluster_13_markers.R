...
# Cluster 13 markers
cluster_markers = read.csv("/home/yasir/Documents/Chapter1/Liger_analysis_strategy_1/Results/Analysis_of_WT_Cardamine/Remove_rep_specific_GEP20_and_22/Cluster_biomarkers/Cluster_specific_markers/Cluster_13_bio_markers.csv", row.names = 1)

cluster_degs = FindMarkers(integrated.data, ident.1 = "13", test.use = "wilcox", only.pos = FALSE)
cluster_degs$gene_ID = rownames(cluster_degs)

cluster_degs = loadRData("Cluster_5_all_degs.RData")
...
geom_text_repel(
  data = subset(markers_description, significance == "sig"),
  aes(label = Gene_code_name),
  size = 5, point.padding = 10
)
...
p <- ggplot(markers_description, aes(x = avg_log2FC, y = log10, color = significance)) +
  ...
  ggrepel::geom_text_repel(
    data = markers_description[(markers_description$significance == "sig") & !is.na(markers_description$TF), ], aes(label = Gene_code_name), nudge_x = -700, size = 16, force = 100
    ) +
  xlab("Average log2FC") + 
  ylab("log10(pval.adj)") + 
  scale_color_manual("Markers", values = c("sig" = grp_col[3], "nonsig" = grp_col[8]), labels = c("Specific", "Cluster DEGs")) + 
  theme(
...
ggsave(filename = "Figure_Volcano_RCO.png", plot = p, width = 14, height = 14, dpi = 300)
