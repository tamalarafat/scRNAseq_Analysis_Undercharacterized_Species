...
OX_HVGs = variableGenes_liger(dataObject = OX_1E, var.thresh = 0.1, num.genes = 3000, alpha.thresh = 0.99, tol = 0.0001)
fileGenerator(OX_HVGs, fileName = "CH_1E_3000_HVG.txt")

mean_var_OX_1E = generate_mean_var_liger(OX_1E)

save(mean_var_OX_1E, file = "Mean_Variance_CH_01.RData")
...
p <- ggplot(data = mean_var_OX_1E, aes(x = avgExpression, y = geneVariance)) + 
  ...
  geom_abline(intercept = log10(mean((1 / OX_1E@meta.data[["nCount_RNA"]]))), slope = 1, color = "red") + 
...
ggsave(plot = p, filename = "OX_1E_Liger_HVG_ab.png", width = 12, height = 12, dpi = 300)
...
ggsave(plot = p, filename = "OX_1E_Liger_HVG.png", width = 12, height = 12, dpi = 300)
