...
OX_data <- Read10X(data.dir = "/netscratch/dep_tsiantis/common/scRNAseq/FINAL_datasets_200822/outs_Ox_RNA_2nd_ALL_2_Newest/filtered_feature_bc_matrix/")
...
OX_2E <- CreateSeuratObject(counts = OX_ortho_data, project = "OX_2E", min.features = 200)
...
OX_HVGs = variableGenes_seurat(dataObject = OX_2E, nfeatures = 3000)
fileGenerator(OX_HVGs, fileName = "CH_2E_3000_HVG_seurat.txt")

mean_var_OX_2E = loadRData("/home/ytamal2/Documents/2022/Final_part_PhD_Thesis/Analyses/HVGs_for_each_dataset/Liger/3000/Ortho_datasets/CH_02/Mean_Variance_CH_02.RData")

seurat_mean_var_OX_2E = get_variableGenes_info(OX_2E)
save(seurat_mean_var_OX_2E, file = "Seurat_Mean_Variance_CH_02.RData")

VariableFeaturePlot(OX_2E)

# Define color
grp_col = c("#FD6467", "#00A08A", "#F98400", "#046C9A")

p <- ggplot(data = mean_var_OX_2E, aes(x = avgExpression, y = geneVariance)) + 
  geom_point(alpha = 0.5, color = grp_col[2]) + 
  geom_point(data = mean_var_OX_2E[OX_HVGs, ], aes(x = avgExpression, y = geneVariance), color = grp_col[3], alpha = 0.5) + 
...
ggsave(plot = p, filename = "OX_2E_Seurat_HVG_sm.png", width = 12, height = 12, dpi = 300)
...
ggsave(plot = p, filename = "OX_2E_Seurat_HVG.png", width = 12, height = 12, dpi = 300)
