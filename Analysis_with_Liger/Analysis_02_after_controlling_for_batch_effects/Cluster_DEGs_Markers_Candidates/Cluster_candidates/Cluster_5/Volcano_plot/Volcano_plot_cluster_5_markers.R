...
# Cluster 5 markers
cluster_markers = read.csv("/home/yasir/Documents/Chapter1/Liger_analysis_strategy_1/Results/Analysis_of_WT_Cardamine/Remove_rep_specific_GEP20_and_22/Cluster_biomarkers/Cluster_specific_markers/Cluster_5_bio_markers.csv", row.names = 1)

cluster_degs = FindMarkers(integrated.data, ident.1 = "5", test.use = "wilcox", only.pos = FALSE)
...
p <- ggplot(markers_description, aes(x = avg_log2FC, y = log10, color = significance)) +
  ...
  ggrepel::geom_text_repel(
    data = markers_description[(markers_description$significance == "sig") & !is.na(markers_description$TF), ], aes(label = Gene_code_name), nudge_x = -600, size = 16, force = 100
    ) +
  xlab("Average log2FC") + 
  ylab("log10(pval.adj)") + 
  scale_color_manual("Markers", values = c("sig" = grp_col[5], "nonsig" = grp_col[8]), labels = c("Specific", "Cluster DEGs")) + 
  theme(
