...
# Specific markers
Cluster_markers = read.delim("Cluster_5_specific_markers_CH_ID.txt", header = FALSE)[, 1]

length(intersect(geneID_chipseq, Cluster_markers)) # 87 (79% match)
...
write.csv(Cluster_markers_chip_subset, file = "Genes_peak/Chip_subset_cluster5_specific_markers.csv", row.names = FALSE)
...
length(unique(Cluster_markers_chip_subset$CH_ID)) # 66
